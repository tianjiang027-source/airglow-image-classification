"""
ConvNeXt-Tiny 预训练微调脚本

数据来源:
  F:\兴隆\训练集\class_train\1
  F:\兴隆\训练集\class_train\1\classification_results.txt

输出:
  F:\兴隆\epoch\convnext_1
"""
...
# ===================== 配置 =====================
TRAIN_SET_DIR = r"F:\兴隆\训练集\class_train\1"
TRAINING_FILE = r"F:\兴隆\训练集\class_train\1\classification_results.txt"
OUTPUT_DIR = r"F:\兴隆\epoch\convnext_2"
...
def normalize_high_bit_image(img):
    ...
    arr = np.clip(arr, 0, FIXED_NORMALIZE_MAX) / FIXED_NORMALIZE_MAX
...
class AstroClassDataset(Dataset):
    def __init__(self, train_set_dir, samples, transform=None):
...
def compute_class_metrics(targets, preds):
    ...
    for target, pred in zip(targets, preds):
        ...
        if target == pred:
            class_true_positive[target] += 1

    recall = [
        class_true_positive[i] / class_total[i] if class_total[i] else 0.0
        for i in range(NUM_CLASSES)
    ]
    precision = [
        class_true_positive[i] / class_predicted[i] if class_predicted[i] else 0.0
        for i in range(NUM_CLASSES)
    ]
...
def save_checkpoint(path, model, epoch, val_acc, a_precision, a_recall):
    torch.save({
...
def main():
    ...
    record_file = os.path.join(RECORD_DIR, f"convnext_training_record_{timestamp}.csv")
    ...
    best_score = -1.0
    best_path = os.path.join(OUTPUT_DIR, "convnext_best.pt")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        epoch_path = os.path.join(OUTPUT_DIR, f"convnext_epoch_{epoch:02d}.pt")
        save_checkpoint(epoch_path, model, epoch, val_acc, a_precision, a_recall)
        print(f"  已保存: {epoch_path}")

        # 更重视 a 类，同时避免只追求 precision 而漏掉大量星空。
        score = (2.0 * a_precision + 2.0 * a_recall + val_acc) / 5.0
        if score > best_score:
            best_score = score
            save_checkpoint(best_path, model, epoch, val_acc, a_precision, a_recall)
            print(f"  ★ 新最佳模型: score={best_score:.4f}")
